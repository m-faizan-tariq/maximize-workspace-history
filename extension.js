...
export default class MaximizeWorkspaceHistory extends Extension {
    constructor(args) {
        ...
        this._excludedApps = ['text-sniper', 'text-sniper.desktop', 'flameshot', 'text-sniper-tray']; // WM_CLASS, app ID, or title keywords to exclude
    }
    ...
    _isExcluded(win) {
        ...
        try {
            const tracker = global.display.get_tracker();
            if (tracker) {
                const app = tracker.get_window_app(win);
                if (app && this._excludedApps.includes(app.get_id().toLowerCase())) {
                    return true;
                }
            }
        } catch (e) {
            // Silently continue if app lookup fails
        }
        return false;
    }
